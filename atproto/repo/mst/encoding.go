...
type NodeData struct {
	Left    *cid.Cid    `cbor:"l" cborgen:"l"` // [nullable] pointer to lower-level subtree to the "left" of this path/key
	Entries []EntryData `cbor:"e" cborgen:"e"` // ordered list of entries at this node
}

// CBOR serialization struct for a single entry within a `NodeData` entry list.
type EntryData struct {
	PrefixLen int64    `cbor:"p" cborgen:"p"` // count of characters shared with previous path/key in tree
	KeySuffix []byte   `cbor:"k" cborgen:"k"` // remaining part of path/key (appended to "previous key")
	Value     cid.Cid  `cbor:"v" cborgen:"v"` // CID pointer at this path/key
	Right     *cid.Cid `cbor:"t" cborgen:"t"` // [nullable] pointer to lower-level subtree to the "right" of this path/key entry
}
