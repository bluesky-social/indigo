...
func (idx *Indexer) deletePost(ctx context.Context, ident *identity.Identity, recordPath string) error {
	...
	logger := idx.logger.With("repo", ident.DID, "path", recordPath, "op", "deletePost")
...
func (idx *Indexer) updateProfilePageranks(ctx context.Context, dids []syntax.DID, ranks []float64) error {
	ctx, span := tracer.Start(ctx, "updateProfilePageranks")
	defer span.End()
	span.SetAttributes(attribute.Int("num_profiles", len(dids)))

	log := idx.logger.With("op", "updateProfilePageranks")
