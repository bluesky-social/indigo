// Helpers for randomly generated gndr.app.* content: accounts, posts, likes,
...
func GenProfile(xrpcc *xrpc.Client, acc *AccountContext, genAvatar, genBanner bool) error {
	...
	_, err := comatproto.RepoPutRecord(context.TODO(), xrpcc, &comatproto.RepoPutRecord_Input{
		...
		Rkey:       "self",
		Record: &lexutil.LexiconTypeDecoder{Val: &appgndr.ActorProfile{
			Description: &desc,
			DisplayName: &name,
			Avatar:      avatar,
			Banner:      banner,
...
func GenPosts(xrpcc *xrpc.Client, catalog *AccountCatalog, acc *AccountContext, maxPosts int, fracImage float64, fracMention float64) error {
	...
	var mention *appgndr.FeedPost_Entity
	...
	for i := 0; i < count; i++ {
		...
		if tgt != nil {
			...
			mention = &appgndr.FeedPost_Entity{
				Type:  "mention",
				Value: tgt.Auth.Did,
				Index: &appgndr.FeedPost_TextSlice{
					Start: 0,
		...
		var images []*appgndr.EmbedImages_Image
		if fracImage > 0.0 && rand.Float64() < fracImage {
			...
			images = append(images, &appgndr.EmbedImages_Image{
				Alt: gofakeit.Lunch(),
		...
		if mention != nil {
			post.Entities = []*appgndr.FeedPost_Entity{mention}
		}
		if len(images) > 0 {
			post.Embed = &appgndr.FeedPost_Embed{
...
func CreateLike(xrpcc *xrpc.Client, viewPost *appgndr.FeedDefs_FeedViewPost) error {
	ctx := context.TODO()
	like := appgndr.FeedLike{
...
func CreateRepost(xrpcc *xrpc.Client, viewPost *appgndr.FeedDefs_FeedViewPost) error {
	repost := &appgndr.FeedRepost{
		CreatedAt: time.Now().Format(time.RFC3339),
		Subject: &comatproto.RepoStrongRef{
			Uri: viewPost.Post.Uri,
			Cid: viewPost.Post.Cid,
	...
	_, err := comatproto.RepoCreateRecord(context.TODO(), xrpcc, &comatproto.RepoCreateRecord_Input{
		Collection: "gndr.app.feed.repost",
		Repo:       xrpcc.Auth.Did,
...
func CreateReply(xrpcc *xrpc.Client, viewPost *appgndr.FeedDefs_FeedViewPost) error {
	text := gofakeit.Sentence(10)
	if len(text) > 200 {
		text = text[0:200]
	}
...
func GenFollowsAndMutes(xrpcc *xrpc.Client, catalog *AccountCatalog, acc *AccountContext, maxFollows int, maxMutes int) error {
	...
	for idx := range rand.Perm(len(catalog.Regulars))[:muteCount] {
		...
		if err := appgndr.GraphMuteActor(context.TODO(), xrpcc, &appgndr.GraphMuteActor_Input{Actor: tgt.Auth.Did}); err != nil {
			return err
		}
	}
	t2(muteCount)
...
func GenLikesRepostsReplies(xrpcc *xrpc.Client, acc *AccountContext, fracLike, fracRepost, fracReply float64) error {
	...
	maxTimeline := 100
	resp, err := appgndr.FeedGetTimeline(context.TODO(), xrpcc, "", "", int64(maxTimeline))
	if err != nil {
		return err
	}
...
func BrowseAccount(xrpcc *xrpc.Client, acc *AccountContext) error {
	// fetch notifications
	maxNotif := 50
	resp, err := appgndr.NotificationListNotifications(context.TODO(), xrpcc, "", int64(maxNotif), false, nil, "")
	if err != nil {
		return err
	}
	...
	for _, notif := range resp.Notifications {
		...
		case "repost":
			fallthrough
		case "follow":
			_, err := appgndr.ActorGetProfile(context.TODO(), xrpcc, notif.Author.Did)
			if err != nil {
				return err
			}
			_, err = appgndr.FeedGetAuthorFeed(context.TODO(), xrpcc, notif.Author.Did, "", "", false, 50)
			if err != nil {
				return err
			}
		case "mention":
			fallthrough
		case "reply":
			_, err := appgndr.FeedGetPostThread(context.TODO(), xrpcc, 4, 80, notif.Uri)
			if err != nil {
				return err
			}
		default:
	...
	// fetch timeline (up to 100), and iterate over posts
	timelineLen := 100
	timelineResp, err := appgndr.FeedGetTimeline(context.TODO(), xrpcc, "", "", int64(timelineLen))
	if err != nil {
		return err
	}
	...
	for _, post := range timelineResp.Feed {
		...
		if rand.Float64() < 0.25 {
			_, err = appgndr.FeedGetPostThread(context.TODO(), xrpcc, 4, 80, post.Post.Uri)
			if err != nil {
				return err
			}
		} else if rand.Float64() < 0.25 {
			_, err = appgndr.ActorGetProfile(context.TODO(), xrpcc, post.Post.Author.Did)
			if err != nil {
				return err
			}
			_, err = appgndr.FeedGetAuthorFeed(context.TODO(), xrpcc, post.Post.Author.Did, "", "", false, 50)
			if err != nil {
				return err
			}
		}
	...
	_, err = appgndr.NotificationGetUnreadCount(context.TODO(), xrpcc, false, "")
	return err
}
