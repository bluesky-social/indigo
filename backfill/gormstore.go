...
type GormDBJob struct {
	...
	State      string `gorm:"index;index:state_retry_after_idx,priority:1;index:state_like_failed_idx,where:state like 'failed%'"`
	Rev        string
	RetryCount int
	RetryAfter *time.Time `gorm:"index:state_retry_after_idx,priority:2"`
}
