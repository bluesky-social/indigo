...
type Scheduler struct {
	...
	do func(context.Context, *JetstreamEvent) error
...
func NewScheduler(maxC, maxQ int, ident string, do func(context.Context, *JetstreamEvent) error) *Scheduler {
	p := &Scheduler{
		maxConcurrency: maxC,
		maxQueue:       maxQ,
...
func (p *Scheduler) AddWork(ctx context.Context, repo string, val *JetstreamEvent) error {
	p.itemsAdded.Inc()
	t := &consumerTask{
		repo: repo,
		val:  val,
