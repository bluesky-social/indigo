...
func (c *CrawlDispatcher) enqueueJobForActor(ai *User) *crawlWork {
	c.maplk.Lock()
	defer c.maplk.Unlock()
	_, ok := c.inProgress[ai.ID]
	if ok {
		return nil
	}
...
func (c *CrawlDispatcher) dequeueJob(job *crawlWork) {
	c.maplk.Lock()
	defer c.maplk.Unlock()
	delete(c.todo, job.act.ID)
	c.inProgress[job.act.ID] = job
}
...
func (c *CrawlDispatcher) addToCatchupQueue(catchup *catchupJob) *crawlWork {
	...
	job, ok = c.inProgress[catchup.user.ID]
	if ok {
...
func (c *CrawlDispatcher) Crawl(ctx context.Context, ai *User) error {
	if ai.PDS == 0 {
...
func (c *CrawlDispatcher) AddToCatchupQueue(ctx context.Context, host *models.PDS, u *User, evt *comatproto.SyncSubscribeRepos_Commit) error {
	if u.PDS == 0 {
		panic("must have pds for user in queue")
	}
