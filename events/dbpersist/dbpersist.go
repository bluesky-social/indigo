...
func (p *DbPersistence) hydrateSync(ctx context.Context, rer *RepoEventRecord) (*events.XRPCStreamEvent, error) {
	did, err := p.didForUid(ctx, rer.Repo)
	if err != nil {
		return nil, err
	}
	...
	out := &comatproto.SyncSubscribeRepos_Sync{
		Seq:    int64(rer.Seq),
		Did:    did,
		Time:   rer.Time.Format(util.ISO8601),
		Rev:    rer.Rev,
	...
	return &events.XRPCStreamEvent{RepoSync: out}, nil
}
